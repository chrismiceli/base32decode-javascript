var Base32Decode = function (base32EncodedString: string): Uint8Array {
    ...
    var alphabet: string = "abcdefghijklmnopqrstuvwxyz234567";
    var returnArray: Array<number> = new Array(base32EncodedString.length * 5 / 8);

    var currentByte: number = 0;
    var bitsRemaining: number = 8;
    var mask: number = 0;
    var arrayIndex: number = 0;

    for (var count: number = 0; count < base32EncodedString.length; count++) {
        var currentIndexValue = alphabet.indexOf(base32EncodedString[count]);
        if (-1 === currentIndexValue) {
            if ("=" === base32EncodedString[count]) {
                var paddingCount: number = 0;
